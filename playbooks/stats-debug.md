...
2. Unexpected `heuristic` on smart paths, or `bm25` when you expected hybrid → `action=llm_status` (`force: true` for a chat ping). Read `fallback_reason` on the last `rerank` / smart result. If URL is unset, run `npx -y compendium-mcp setup-ollama` (playbook `setup-ollama`).
